...
class MCTS:
    ...
    def run(self, model, state, to_play):
        ...
        # EXPAND root
        action_probs, value = model.predict(state[:-1])
        ...
        for _ in range(self.args['num_simulations']):
            ...
            next_state = self.game.get_canonical_board(next_state, player=next_player)
            ...
            if value is None:
                ...
                # EXPAND
                action_probs, value = model.predict(next_state[:-1])
    ...
    def get_action_probs(self, state):
        action_probs, _ = self.model.predict(state[:-1])
        valid_moves = self.game.get_valid_moves(state)
        action_probs = action_probs * valid_moves  # mask invalid moves
        action_probs /= np.sum(action_probs) + 1e-10  # renormalize
        return action_probs
