...
def teams_to_names(board):
    t0 = np.where(board == 1)[0]
    t1 = np.where(board == -1)[0]

    return tuple(sorted([idx_to_name(i) for i in t0])), tuple(sorted([idx_to_name(i) for i in t1]))
